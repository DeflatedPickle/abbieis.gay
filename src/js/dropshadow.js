...
const paint = (p, x, y) => {
  ...
  requestAnimationFrame(() => {
    ...
    recursiveShadow(p, r, nx, ny)
  });
};

function recursiveShadow(p, r, nx, ny) {
  p.childNodes.forEach((d) => {
    if (p instanceof HTMLDivElement && d.style && !d.classList.contains("noshadow")) {
      const o2 = Math.min(r.width, r.height, 12);
      d.style.filter = `drop-shadow(${-1 * nx * o2}px ${-1 * ny * o2}px 0px rgba(0, 0, 0, 0.1))`;

      recursiveShadow(d, r, nx, ny);
    }
  });
}
